...
class AgentService {
    ...
    // send new schema to the ledger
    async sendNewSchema(newSchema) {
        try {
            await httpAsync({
                hostname: hostname,
                port: port,
    ...
    // get all of the schemas created by the user
    async getMyCreatedSchemas() {
        try {
            await httpAsync({
                hostname: hostname,
                port: port,
    ...
    // send to new credential definition to the ledger
    async sendNewCredentialDefinition(newCredentialDefinition) {
        try {
            await httpAsync({
                hostname: hostname,
                port: port,
                //TODO: add api call to the path
                path: '/',
                method: 'POST'
